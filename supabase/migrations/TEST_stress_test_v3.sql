-- =============================================================================
-- 🔥 STRESS TEST SUITE v3 - FIXED FOR YOUR SCHEMA
...
BEGIN
  ...
  INSERT INTO _stress_test_results VALUES (0, 'SETUP', 'Find resources', 'Found', '✅ PASSED', 
    'Product: ' || LEFT(v_product_id::TEXT, 8) || '..., Unit: ' || LEFT(v_unit_id::TEXT, 8) || '...');

  -- Create test customers (simple INSERT, delete first if exists)
  DELETE FROM customers WHERE email IN ('stress-test-1@example.com', 'stress-test-2@example.com');
  ...
  ELSE
    INSERT INTO _stress_test_results VALUES (3, '🔥 RACE: Double-book Crew', 'N/A', 'Skipped', '⏭️ SKIPPED', 
      'Only 1 unit available - cannot test crew conflict separately');
  END IF;
  ...
  ELSE
    INSERT INTO _stress_test_results VALUES (8, 'Slot Freed After Cancel', 'is_available=true', 
      'is_available=false', '❌ FAILED', 'Slot still blocked: ' || COALESCE(v_avail.unavailable_reason, 'Unknown'));
  END IF;
