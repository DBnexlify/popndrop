-- =============================================================================
-- FIX: Include 'pending' status in availability checks
-- 
-- PROBLEM: Slot availability functions only checked confirmed+ statuses,
--          ignoring pending bookings. This allowed double-booking during
--          the checkout window when bookings are still pending.
--
-- SOLUTION: Include 'pending' status in all availability checks, but add
--           a 45-minute timeout so abandoned pending bookings don't block forever.
-- =============================================================================
  ...
  WHERE r.resource_type = p_resource_type
    ...
    AND NOT EXISTS (
      ...
      WHERE bb.resource_type = 'ops'
        ...
        -- FIX: Include pending status (soft holds)
        -- Pending bookings only block if created within last 45 minutes
        AND (
          b.status IN ('confirmed', 'delivered', 'picked_up', 'completed')
          OR (b.status = 'pending' AND b.created_at > NOW() - INTERVAL '45 minutes')
        )
...
BEGIN
  ...
  SELECT 
    ...
    CASE
      ...
      -- Asset availability check - FIX: Include pending (soft holds)
      WHEN EXISTS (
        SELECT 1 
        FROM booking_blocks bb
        JOIN bookings b ON b.id = bb.booking_id
      ...
      -- Delivery crew check (already uses updated count_available_ops_resources)
    ...
    CASE
      ...
      -- Asset check reason - FIX: Include pending
      WHEN EXISTS (
        SELECT 1 
        FROM booking_blocks bb
        JOIN bookings b ON b.id = bb.booking_id
...
BEGIN
  ...
  -- FIX: Include pending status (soft holds) in unit availability check
  ...
  -- find_available_ops_resource is already updated to include pending
...
DO $$
BEGIN
  RAISE NOTICE '=== PENDING STATUS FIX APPLIED ===';
  RAISE NOTICE '';
  RAISE NOTICE 'WHAT WAS FIXED:';
  RAISE NOTICE '  All availability functions now include pending bookings (soft holds)';
  RAISE NOTICE '  Pending bookings block slots for 45 minutes (checkout window)';
  RAISE NOTICE '';
  ...
  RAISE NOTICE '';
  RAISE NOTICE 'Party House and Bounce House bookings should now work correctly!';
END $$;
